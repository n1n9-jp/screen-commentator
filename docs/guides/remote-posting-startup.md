...
`email rate limit exceeded`が出た場合は、Supabase Authのメール送信制限に達しています。デフォルトでは同じメールアドレスへのOTP再送は約60秒待つ必要があり、プロジェクト全体のOTP送信数にも上限があります。まず60秒以上待ってから再送してください。検証中に頻発する場合は、Supabase Dashboardの`Authentication` -> `Rate Limits`でOTP送信制限を確認します。Supabase公式ドキュメント上、`Send One-Time-Passwords (OTP)`はデフォルトで30 OTP/時、`Send OTPs or magic links`は同一ユーザーへの再送がデフォルト60秒間隔です。短時間に何度も再送せず、最新メールの6桁コードだけを使います。
...
- `Authentication`または`Auth` -> `URL Configuration`
- `Site URL`: 本番で使うVercel URL。例: `https://screen-commentator-web.vercel.app`
- `Redirect URLs`: 本番URLに`/**`を付けて追加。例: `https://screen-commentator-web.vercel.app/**`
